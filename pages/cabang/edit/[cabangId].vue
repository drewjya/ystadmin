...
const apiUrl = computed(() => apiPath().getCabangById(cabangId));
...
const initState = async () => {
  ...
  const data = await fetch(runtime.public.baseUrl + apiUrl.value, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${app.accessToken ?? "invalid_token"}`,
    },
  });
  const json: SResponse<any> = await data.json();
  console.log(json);
  if (json.status < 400) {
    formD.state.name = json.data.nama;
    formD.state.alamat = json.data.alamat;
    formD.state.phoneNumber = json.data.phoneNumber;
    formD.state.openHour = extractHourMinutesDate(json.data.openHour);

    formD.state.closeHour = extractHourMinutesDate(json.data.closeHour);

    if (json.data.picture) {
      const response = await fetch(
        runtime.public.imageUrl + json.data.picture,
        {
          method: "GET",
        }
      );
      ...
      const file = new File([data], json.data.picture, {
        type: data.type || "image/png",
      });
      if (file) {
        formD.state.file = file;
        url.value = URL.createObjectURL(file);
      }
    }
  }
  console.log(formD.state);
...
const formD = useFormd({
  ...
  onSubmit: async (event, d) => {
    ...
    const data = await fetch(
      runtime.public.baseUrl + apiPath().updateCabang(cabangId),
      {
        method: "PUT",
        body: formData,
        headers: {
