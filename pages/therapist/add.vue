...
const initState = async () => {
  const data = await fetch(runtime.public.baseUrl + apiUrl.getCabang(), {
    method: "GET",
    headers: {
      Authorization: `Bearer ${app.accessToken ?? "invalid_token"}`,
    },
  });
  const json: SResponse<Cabang[]> = await data.json();
  console.log(json);
  if (json.status < 400) {
    if (!json.data) {
      notify.info({
...
const formD = useFormd({
  ...
  onSubmit: async (event, d) => {
    const data = await usePrivateApi(
      runtime.public.baseUrl + path.postTherapist(),
      {
        method: "POST",
        body: event.data,
      }
...
<template>
  <v-form :form="formD" class="flex flex-col gap-4">
    ...
    <u-form-group label="Gender" name="gender">
      <u-select-menu
        v-model="formD.state.gender"
        :options="['MALE', 'FEMALE']"
      ></u-select-menu>
    </u-form-group>
